...
async def test_add_and_get_hotel(authenticated_ac: AsyncClient):
    response = await authenticated_ac.post("/hotels", json={
        "name": "test_hotel",
...
async def test_get_hotel_by_location(location, date_from, date_to, authenticated_ac: AsyncClient):
...
async def test_get_hotel_rooms(hotel_id, date_from, date_to, authenticated_ac: AsyncClient):
