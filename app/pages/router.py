...
from app.bookings.router import get_all_bookings
...
async def get_hotels_page_with_location(
    request: Request,
    location: str,
    date_to: date,
    date_from: date,
...
async def get_successful_booking_page(
    ...
    await BookingDAO.add(
        user_id=user.id, date_from=date_from, date_to=date_to, room_id=room_id
    )
