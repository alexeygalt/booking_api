...
class HotelsDAO(BaseDAO):
    ...
    async def add(cls, hotel_data: SHotelCreate):
        async with async_session_maker() as session:
            query = (
                insert(cls.model).values(**hotel_data.model_dump()).returning(Hotels)
            )
            ...
            booked_rooms = (
                (
                    select(
                        ...
                        Bookings.room_id,
                        func.count(Hotels.id).label("booked_rooms"),
                    )
                    .select_from(Hotels)
                ...
                .group_by(Hotels.id, Bookings.room_id)
                .cte("booked_rooms")
            )
    ...
    async def get_hotel_rooms(cls, hotel_id: int, date_from: date, date_to: date):
        async with async_session_maker() as session:
            booked_rooms = (
                select(Bookings.room_id, func.count(Rooms.id).label("booked_count"))
                .select_from(Rooms)
                .join(Bookings, Rooms.id == Bookings.room_id)
                .where(
                    or_(
            ...
            filtered_rooms = (
                ...
                .select_from(Rooms)
                .join(booked_rooms, booked_rooms.c.room_id == Rooms.id, full=True)
